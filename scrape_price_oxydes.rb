...
def get_prices_and_formats
  ...
  filepath = '/Users/Laf1BUp/Dropbox/Documents/Noblessence/Noblessence_Ressources_Site_web/Data_votresite/To_import/products-stripped-oxydes.csv'
  ...
  CSV.foreach(filepath) do |row|
    ...
    url = "http://www.noblessence.com/fr/produits/matieres-premieres/c246197/#{r}.html"
    ...
    begin
      ...
      if html_file
        ...
        oxyde_name = html_doc.xpath('//*[@id="item_desc2"]/h2').children.text
...
def write_to_csv(data)
  file = '/Users/Laf1BUp/Dropbox/Documents/Noblessence/Noblessence_Ressources_Site_web/Data_votresite/To_import/oxydes-data.csv'

  # CSV.open(file, 'a', write_headers: true, headers: ['Name', 'Url', 'Format', 'Price']) do |csv|
  CSV.open(file, 'a') do |csv|
    # csv << ['Name', 'Url', 'Format', 'Price']
    data.each do |oxyde|
        if oxyde.class == String || oxyde[-1].class == String 
          hash = {}
          hash[:name] = oxyde
          csv << hash.values
        else
